...
namespace gazebo {
   // double init_x, init_y;
	...
	class KivaRotateCenter: public ModelPlugin {
		public: KivaRotateCenter(): ModelPlugin() {}
		public: virtual void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {

		    if (!ros::isInitialized())
		    {
		      ROS_ERROR("A ROS node for Gazebo has not been initialized, unable to load plugin. Load the Gazebo system plugin 'libgazebo_ros_api_plugin.so' in the gazebo_ros package");
		      return;
		    } else {
			ROS_INFO("PLUGIN ACCESS COMPLETE");		
			}
    ...
    private: void OnMsg(geometry_msgs::Vector3 _msg){
        int kivaId = _msg.z;
        if(model->GetName().compare("kiva_"+ std::to_string(kivaId))==0){
