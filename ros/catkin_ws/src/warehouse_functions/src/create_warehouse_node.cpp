...
void spawn_model(ros::NodeHandle n, std::string model_name, std::string model_sdf, int pos_x, int pos_y) {
	//ros::NodeHandle n;
...
void createWarehouseCallback(warehouse_functions::create_warehouse_msg_request create_request) {
	...
	while(no_of_robots) {
		...
		//spawn_model(model_name, ROBOT_MODEL_SDF, pos_x, pos_y);	
		no_of_robots--;
		pos_x++;
	}
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "create_warehouse_node");
	ros::NodeHandle n;
	//ros::Subscriber create_warehouse_sub = n.subscribe("/warehouses/create_warehouse_models", 10, createWarehouseCallback);

	//spawn_model(n, "kiva", ROBOT_MODEL_SDF, 0, 0);
	int no_of_robots = 2;
	int no_of_storage_units = 10;
	std::string warehouse_name = "warehouse";
